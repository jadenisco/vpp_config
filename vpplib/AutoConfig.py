...
class AutoConfig(object):
    ...
    def _loadconfig(self):
        ...
        if self._clean is False and os.path.isfile(systemfile):
            with open(systemfile, 'r') as sysstream:
                systopo = yaml.load(sysstream, Loader = yaml.FullLoader)
                if 'nodes' in systopo:
    ...
    def _update_auto_config(self):
        ...
        nodes = {}
        with open(self._autoconfig_filename, 'r') as stream:
            ydata = yaml.load(stream, Loader = yaml.FullLoader)
            if 'nodes' in ydata:
                nodes = ydata['nodes']
    ...
    def apply_grub_cmdline(self):
        ...
        for i in self._nodes.items():
            ...
            if 'grub' not in node:
                logging.debug("Grub not used on node {}".format(node['host']))
                continue
    ...
    def sys_info(self):
        ...
        for i in self._nodes.items():
            ...
            if 'grub' in node:
                ...
                print("  Grub not used on this node.")
